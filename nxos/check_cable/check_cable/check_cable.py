...
def do_work(switches, blueprint, method):
    ...
    actualNeighbors = PrettyTable(['LocalSw', 'LocalInt','RemoteSw','RemoteInt'])
    print "######################## Actual Neighbor Information #############################"
    for i in range(0, len(connectivities)):
        #print "******************************************************************"
        for j in range(0, len(connectivities[i])):
            items = connectivities[i][j].split(";")
            actualNeighbors.add_row([items[0],items[1],items[2],items[3]])
    ...
    conn_obj = jsoncfg.loads(conn_db)
    conn_db_list = list()
    nes = conn_obj["ne"]
    ...
    match_set = set()
    conformanceTable = PrettyTable(['LocalSw', 'LocalInt','RemoteSw','RemoteInt','Result'])
        ...
        for j in range(0, len(connectivities)):
            for k in range(0, len(connectivities[j])):
                ...
                if conn_db_list[i] == connectivities[j][k]:
                    ...
                    conformanceTable.add_row([items[0],items[1],items[2],items[3],'Pass'])
                    match_set.add(i)
    ...
    for i in range(0, len(conn_db_list)):
        ...
        conformanceTable.add_row([items[0],items[1],items[2],items[3],'Fail'])
    print conformanceTable
